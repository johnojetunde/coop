...
if($gender=="male"){
$images = array("avatar.png","avatar04.png","avatar5.png");

}
else {
$images = array("avatar2.png","avatar3.png","user.jpg","user2.jpg");
}
$limit = sizeof($images);
$image = rand(0,$limit-1);
$img = $images[$image];
return $img;
}
$gender = "male";
//this is where you that of images. You have to edit it here. 
$username = $res['username'];
$sql =" SELECT * FROM users WHERE `username`='{$username}' LIMIT 1";
            $result =$this->db->query($sql)->fetchAll();
			$string = $_SERVER['HTTP_REFERER'];
			$pos = strpos($string,"admin");
			if($pos===false){
			$attach = "dp/";
			...
			if($result){
			$show_passport = $result[0]['passport'];
			$passport = $attach.$show_passport;
			}else{
			$passport = $attach."admin.png";
			}
			/*
			$length = strlen($result);
			if($length<=0){
			$show_passport = "admin.png";
			}else{
			$show_passport = $result[0]['passport'];
			}
			//print_r($result);
			...
			$passport = $attach.$show_passport;
			*/
			//$passport = $_SERVER['HTTP_REFERER'];
			//$passport = "".$res['username'];
            //$stmt->execute(array("time" => $time));
            //$result = $stmt->fetchAll();
//$passport = $result[0]['passport'];
//$show_pass = "dp/".$passport;

            $avatar_url = $passport; //$this->url . "/client/jquery/user.jpeg";
        ...
        foreach ($cres as $res) {
            ...
            $avatar_url = $_SESSION['passport']; //$this->url . "/client/jquery/user.jpeg";
